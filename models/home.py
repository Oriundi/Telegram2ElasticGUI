...
def get_chats():
    ...
    data = conn_raw.search(body=req, index='messages')['aggregations']['unique_names']['buckets']
    chats = list()
    for value in data:
...
def get_messages(date_from, condition='AND', search_words=None, username=None, chat=None, limit=10):
    ...
    if limit == 0:
        data = conn_raw.search(index='messages', query=sql, size=10000, sort='timestamp:desc')
    else:
        data = conn_raw.search(index='messages', query=sql, size=limit, sort='timestamp:desc')
    df = pd.DataFrame()
